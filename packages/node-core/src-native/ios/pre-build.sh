...
CWD="$SRCROOT/../../../../crates/platforms/ios"
pushd "$SRCROOT/../../../.."
...
cbindgen --config "$CWD/cbindgen.toml"  "$CWD/src/lib.rs" -l c >"$SRCROOT/NodeCompat/include/node_compat.h"

cargo +nightly build -Z build-std='std,panic_abort' --manifest-path Cargo.toml --target $RUST_BUILD_TARGET $RUST_BUILD_TYPE -p node-ios
popd
