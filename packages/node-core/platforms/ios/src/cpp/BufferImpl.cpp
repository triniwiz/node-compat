...
BufferImpl::BufferImpl(Buffer *buffer) : buffer_(buffer) {

    auto slice = buffer_buffer(buffer_);
    auto length = buffer_length(buffer_);
    auto clone = buffer_clone(buffer_);

    this->store_ = v8::ArrayBuffer::NewBackingStore(static_cast<void *>(slice),
                                                    length, [](void *data, size_t length,
                                                               void *deleter_data) {
...
void BufferImpl::Alloc(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (count == 2) {
        ...
        if (fill->IsString() || fill->IsStringObject()) {
            ...
            auto buffer = buffer_alloc_with_size_string_encoding(size, fillValue.c_str(),
...
void BufferImpl::Atob(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (value->IsString() || value->IsStringObject()) {
        ...
        auto ret = buffer_atob(str.c_str());
...
void BufferImpl::Btoa(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (value->IsString() || value->IsStringObject()) {
        auto str = Helpers::ConvertFromV8String(isolate, value);
        auto ret = buffer_btoa(str.c_str());
        args.GetReturnValue().Set(
                Helpers::ConvertToV8String(isolate, ret)
        );
        return;
...
void BufferImpl::Concat(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (value->IsArray()) {
        ...
        if (concatLength != -1) {
            auto buffer = buffer_concat_length(vec.data(), vec_lengths.data(), vec_lengths.size(),
                                               concatLength);
            bufferImpl = new BufferImpl(buffer);
        } else {
            auto buffer = buffer_concat(vec.data(), vec_lengths.data(), vec_lengths.size());
            bufferImpl = new BufferImpl(buffer);
        }
...
void BufferImpl::CopyBytesFrom(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (value->IsTypedArray()) {
        ...
        auto buf = buffer_from_slice(data, length);
...
void BufferImpl::From(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (value->IsString() || value->IsStringObject()) {
        ...
        StringEncoding encoding = Helpers::ParseEncoding(isolate, encodingValue,
                                                         StringEncoding::StringEncodingUtf8);
        auto str = Helpers::ConvertFromV8String(isolate, value);
        auto buffer = buffer_from_string(str.c_str(), encoding);
        ...
        auto buffer = buffer_from_slice(data, arrayBuffer->ByteLength());
        ...
        auto buffer = buffer_from_slice(vector.data(), vector.size());
...
void BufferImpl::ToString(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    StringEncoding encoding = StringEncoding::StringEncodingUtf8;
    intptr_t start = 0;
    intptr_t end = -1;
    if (ptr != nullptr) {
        ...
        if (len > 0) {
            encoding = Helpers::ParseEncoding(isolate, encodingValue,
                                              StringEncoding::StringEncodingUtf8);
        }

        if (len > 1 && startValue->IsNumber()) {
            start = (intptr_t) startValue->NumberValue(ctx).ToChecked();
        }

        if (len > 2 && endValue->IsNumber()) {
            end = (intptr_t) endValue->NumberValue(ctx).ToChecked();
        }
...
void BufferImpl::WriteInt8(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    if (ptr != nullptr) {
        ...
        if (offsetValue->IsNumber()) {
            offset = (intptr_t) offsetValue->NumberValue(ctx).ToChecked();
        }
        if (value->IsNumber()) {
...
void BufferImpl::IndexedGetter(uint32_t index, const v8::PropertyCallbackInfo<v8::Value> &info) {
    ...
    if (ptr != nullptr) {
        auto buffer = buffer_buffer(ptr->buffer_);
        auto size = buffer_length(ptr->buffer_);
        if (buffer == nullptr) {
            info.GetReturnValue().SetUndefined();
            return;
        }
...
void BufferImpl::IndexedSetter(uint32_t index, v8::Local<v8::Value> value,
    ...
    if (ptr != nullptr) {
        ...
        if (((size_t) index) < length && value->IsNumber()) {
