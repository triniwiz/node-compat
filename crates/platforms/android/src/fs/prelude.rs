...
pub fn to_integer<'a>(env: &mut JNIEnv<'a>, value: c_int) -> JObject<'a> {
    let integer = find_class(INTEGER_CLASS).unwrap();
    env.new_object(integer, "(I)V", &[value.into()]).unwrap()
}

pub fn to_long<'a>(env: &mut JNIEnv<'a>, value: c_int) -> JObject<'a> {
    let integer = find_class(LONG_CLASS).unwrap();
    env.new_object(integer, "(J)V", &[value.into()]).unwrap()
}

pub fn to_float<'a>(env: &mut JNIEnv<'a>, value: c_float) -> JObject<'a> {
    let integer = find_class(FLOAT_CLASS).unwrap();
    env.new_object(integer, "(F)V", &[value.into()]).unwrap()
}

pub fn to_double<'a>(env: &mut JNIEnv<'a>, value: c_double) -> JObject<'a> {
    let integer = find_class(DOUBLE_CLASS).unwrap();
    env.new_object(integer, "(D)V", &[value.into()]).unwrap()
}
...
pub fn error_to_jstring(val: std::io::Error) -> GlobalRef {
    ...
    //
    env.new_global_ref(env.new_string(val.to_string()).unwrap())
        .unwrap()
}
