...
use crate::fs::FILE_STAT_CLASS;
use crate::fs::prelude::*;


pub fn build_stat<'a>(env: &mut JNIEnv<'a>, stat: FileStat) -> JObject<'a> {
    ...
    let _
        = env.set_field(&object, "birthtimeMs", "D", stat.birthtimeMs.into());
...
pub fn build_stat_op<'a>(env: &mut JNIEnv<'a>, stat: Option<NonNull<FileStat>>) -> JObject<'a> {
